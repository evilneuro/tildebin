...
my $logfile = "/var/log/apache/*access*log* /home/support/neuro/host/logs/*access*log*";
## -- file to print report to (make sure this ends in .html)  --
my $outfile = "/var/www/cr/report.html";
## -- file to print number of attacks to --
my $stat    = "/var/www/cr/attacks";
## -- whether or not to be silent --
my $silent = 0;
## -- whether or not to use DNS cache --
my $use_dns_cache = 1;
## -- DNS cache file name to save data to--
my $dns_cache_file = "/var/www/cr/.woreport.dnscache";
...
(
	{
		PATTERN		=>	'^(.*?)\s+(.*?)\[(.*?)\](.*?)\.ida\?N',
		IDENTIFY	=>	'V1',
		NAME		=>	'CodeRed Version 1',
		COLOR		=>	'#00AA00'
	},
	{
		PATTERN		=>	'^(.*?)\s+(.*?)\[(.*?)\](.*?)\.ida\?X',
		IDENTIFY	=>	'V2',
		NAME		=>	'CodeRed Version 2',
		COLOR		=>	'#FF0000'
	},
	{
		PATTERN		=>	'^(.*?)\s+(.*?)\[(.*?)\](.*?)\.ida\?(.*?)',
		IDENTIFY	=>	'V?',
		NAME		=>	'CodeRed Version Unknown',
		COLOR		=>	'#AAAABB'
	},
	{
...
sub print_attacks
{
	...
	print OUT "<html><head><title>Code Red Worm statistics</title></head><body>";
	...
	for(my $v = 0; $v < @SIGNATURES; $v++)
	{
		...
		print OUT " " . $SIGNATURES[$v]->{NAME} . " attacks";;
	}
	print OUT "</td></tr>";
